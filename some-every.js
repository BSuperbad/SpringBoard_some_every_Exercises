...
function hasNoDuplicates(arr) {
    return arr.every(function (value, index) {
        //we set parameters to be a value and the index of that value
        return arr.indexOf(value) === index;
        //indexOf method is called on the input array with the current value as 
        //argument then returns index of the first occurance of value in array
        //indexOf result is compared with current index. If the two values are 
        //equal, it means current value is not a duplicate
        //If the two values are not equal, it means that the current value is a 
        //duplicate, and the every method stops processing the array and returns false.
    });
}
